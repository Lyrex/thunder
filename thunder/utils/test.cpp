...
int main( int, char** )
{
    ...
    std::vector<std::string_view> res_view;

    auto start_time = std::chrono::high_resolution_clock::now();
    for (auto i = 0; i < 10000; i++)
        res_view = thunder::utils::string::between(test_1, "\"", "\"", true, true);

    auto end_time = std::chrono::high_resolution_clock::now();
    
    std::cout << "Found " << res_view.size() << " elements!\n";
...
//		std::cout << element.c_str() << "\n";
    ...
    for (auto i = 0; i < 100000; i++)
        res_view = thunder::utils::string::split(test_1, "\"");
