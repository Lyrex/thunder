	...
	namespace utils
	{
		namespace string
		{
			std::vector<std::string> string_between( const std::string& s_, std::string start_marker, std::string end_marker, bool ignore_case )
			{
				...
				if ( ignore_case )
				{
					std::transform( start_marker.begin(), start_marker.end(), start_marker.begin(), std::tolower );
					std::transform( end_marker.begin(), end_marker.end(), end_marker.begin(), std::tolower );
				}
